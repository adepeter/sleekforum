...
from ..forms.password import PasswordResetForm, SetPasswordForm

TEMPLATE_URL = 'users/auth/password'


class AuthPasswordReset(PasswordResetView):
    success_url = reverse_lazy('sleekforum:users:auth:password_reset_done')
    ...
    def get(self, request, *args, **kwargs):
        if request.user.is_authenticated:
            return HttpResponseRedirect(reverse('sleekforum:home:home'))
...
class AuthPasswordResetConfirm(PasswordResetConfirmView):
    ...
    success_url = reverse_lazy('sleekforum:users:auth:password_reset_complete')
...
class AuthPasswordResetComplete(PasswordResetCompleteView):
    ...
    def get(self, request, *args, **kwargs):
        self.password_reset_complete = self.get_session()
        return super().get(request, *args, **kwargs)

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['password_reset_complete'] = self.password_reset_complete
        return context
